...
float CalcShadowSpotDir (vec4 fragLightSpacePos) {
...
void main () {
    ...
    if (BPLight_type == 0) {
        ...
        // Directional light
        color = vec4(CalcLighting(-BPLight_dir, normView, vsOut.normal, BPLight_brightness, Material_color, Material_specular_color, Material_alpha, BPLight_color, CalcShadowSpotDir(vsOut.fragLightSpacePos)), 1.0);
    } else {
        // Spot light
        ...
        color = vec4(CalcLighting(normLight, normView, vsOut.normal, attenuation, Material_color, Material_specular_color, Material_alpha, BPLight_color, CalcShadowSpotDir(vsOut.fragLightSpacePos)), 1.0);
    }
