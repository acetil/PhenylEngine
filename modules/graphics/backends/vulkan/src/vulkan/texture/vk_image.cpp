...
VulkanImage::VulkanImage (VulkanResources& resources, VkFormat format, VkImageAspectFlags aspect,
    VkImageUsageFlags usage, std::uint32_t width, std::uint32_t height, std::uint32_t layers) :
    m_format{format},
    m_aspect{aspect},
    m_width{width},
    m_height{height},
    ...
    PHENYL_ASSERT_MSG(width > 0 && height > 0, "Attempted to create vulkan image with invalid dimensions: {}x{}", width,
        height);
...
void VulkanImage::loadImage (TransferManager& transferManager, const graphics::Image& image, std::uint32_t layer) {
    ...
    PHENYL_ASSERT_MSG(width() == image.width() && height() == image.height(),
        "Attempted to load image with unexpected dimensions: expected {}x{}, got {}x{}", width(), height(),
        image.width(), image.height());
    PHENYL_ASSERT_MSG(layer < layers(), "Attempted to load image to invalid layer: {} (num layers: {})", layer,
        layers());
...
void VulkanImage::copy (TransferManager& transferManager, VulkanImage& srcImage) {
    ...
    PHENYL_ASSERT_MSG(width() == srcImage.width() && height() == srcImage.height(),
        "Attempted to copy image with incorrect dimensions, this: {}x{}, src: {}x{}", width(), height(),
...
CombinedSampler::CombinedSampler (VulkanResources& resources, const graphics::TextureProperties& properties,
    VkImageLayout samplerLayout) :
    m_sampler{resources, properties},
    m_samplerLayout{samplerLayout} {}

CombinedSampler::CombinedSampler (VulkanResources& resources, const graphics::FrameBufferProperties& properties,
    VkImageLayout samplerLayout) :
    m_sampler{resources, properties},
    m_samplerLayout{samplerLayout} {}
