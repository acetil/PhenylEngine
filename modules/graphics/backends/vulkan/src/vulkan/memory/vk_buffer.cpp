...
VulkanBuffer::VulkanBuffer (VulkanResources& resources, VkBufferUsageFlags usage, std::size_t bufSize) : m_size{bufSize} {
...
void VulkanBuffer::copyIn (std::span<const std::byte> data, std::size_t off) {
    ...
    auto result = vmaCopyMemoryToAllocation(m_bufferInfo->allocator, data.data(), m_bufferInfo->alloc, static_cast<VkDeviceSize>(off), static_cast<VkDeviceSize>(data.size()));
    PHENYL_ASSERT_MSG(result == VK_SUCCESS, "Failed to copy memory to allocation: {}", result);

    result = vmaFlushAllocation(m_bufferInfo->allocator, m_bufferInfo->alloc, static_cast<VkDeviceSize>(off), static_cast<VkDeviceSize>(data.size()));
    PHENYL_ASSERT_MSG(result == VK_SUCCESS, "Failed to flush memory alocation: {}", result);
}

VulkanStaticBuffer::VulkanStaticBuffer (VulkanResources& resources, TransferManager& transferManager, VkBufferUsageFlags usage, std::span<const std::byte> data) : m_size{data.size()} {
