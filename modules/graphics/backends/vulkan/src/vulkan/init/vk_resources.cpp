...
VulkanResources::VulkanResources (VkInstance instance, VulkanDevice& device, std::uint64_t maxFramesInFlight) : m_device{device},
        m_allocator{device.makeVmaAllocator(instance, VK_API_VERSION_1_3)}, m_maxFramesInFlight{maxFramesInFlight},
        m_bufferQueue{[&] (VulkanBufferInfo info) { vmaDestroyBuffer(m_allocator, info.buffer, info.alloc); }},
        m_descriptorSetLayoutQueue{[&] (VkDescriptorSetLayout layout) { vkDestroyDescriptorSetLayout(device.device(), layout, nullptr); }},
        m_pipelineLayoutQueue{[&] (VkPipelineLayout layout) { vkDestroyPipelineLayout(device.device(), layout, nullptr); }},
        m_pipelineQueue{[&] (VkPipeline pipeline) { vkDestroyPipeline(device.device(), pipeline, nullptr); }},
        m_semaphoreQueue{[&] (VkSemaphore semaphore) { vkDestroySemaphore(device.device(), semaphore, nullptr); }},
        m_fenceQueue{[&] (VkFence fence) { vkDestroyFence(device.device(), fence, nullptr); }},
        m_commandPoolQueue{[&] (VkCommandPool pool) { vkDestroyCommandPool(device.device(), pool, nullptr); } },
        m_commandBufferQueue{[&] (VulkanCommandBufferInfo info) { vkFreeCommandBuffers(device.device(), info.pool, 1, &info.commandBuffer); } },
        m_imageQueue{[&] (VulkanImageInfo&& info) { vmaDestroyImage(m_allocator, info.image, info.alloc); }},
        m_imageViewQueue{[&] (VkImageView&& view) { vkDestroyImageView(device.device(), view, nullptr); }},
        m_samplerQueue{[&] (VkSampler&& sampler) { vkDestroySampler(device.device(), sampler, nullptr); } } {}
...
VulkanResource<VulkanBufferInfo> VulkanResources::makeBuffer (const VkBufferCreateInfo& createInfo, const VmaAllocationCreateInfo& allocCreateInfo) {
    VkBuffer buffer;
    VmaAllocation alloc;
    if (auto result = vmaCreateBuffer(m_allocator, &createInfo, &allocCreateInfo, &buffer, &alloc, nullptr); result != VK_SUCCESS) {
    ...
    return {VulkanBufferInfo{
        ...
        .buffer = buffer,
        .alloc = alloc
    }, [&] (VulkanBufferInfo&& info) { m_bufferQueue.queueDestruction(info, getDestructionFrame()); }};
}
...
VulkanResource<VulkanImageInfo> VulkanResources::makeImage (const VkImageCreateInfo& createInfo,
    ...
    VkImage image;
    VmaAllocation alloc;
    if (auto result = vmaCreateImage(m_allocator, &createInfo, &allocCreateInfo, &image, &alloc, nullptr); result != VK_SUCCESS) {
    ...
    return {VulkanImageInfo{ .image = image, .alloc = alloc }, [&] (VulkanImageInfo&& info) { m_imageQueue.queueDestruction(info, getDestructionFrame()); }};
}
...
VulkanResource<VkImageView> VulkanResources::makeImageView (const VkImageViewCreateInfo& createInfo) {
    ...
    return {imageView, [&] (auto&& view) { m_imageViewQueue.queueDestruction(view, getDestructionFrame()); }};
}
...
VulkanResource<VkSampler> VulkanResources::makeSampler (const VkSamplerCreateInfo& createInfo) {
    ...
    return {sampler, [&] (auto&& s) { m_samplerQueue.queueDestruction(s, getDestructionFrame()); } };
}
...
VulkanResource<VkDescriptorSetLayout> VulkanResources::makeDescriptorSetLayout (const VkDescriptorSetLayoutCreateInfo& layoutInfo) {
    ...
    if (auto result = vkCreateDescriptorSetLayout(m_device.device(), &layoutInfo, nullptr, &descriptorSetLayout); result != VK_SUCCESS) {
    ...
    return {descriptorSetLayout,
        [&] (VkDescriptorSetLayout&& layout) { m_descriptorSetLayoutQueue.queueDestruction(layout, getDestructionFrame()); } };
}

VulkanResource<VkPipelineLayout> VulkanResources::makePipelineLayout (const VkPipelineLayoutCreateInfo& layoutInfo) {
    VkPipelineLayout pipelineLayout;
    if (auto result = vkCreatePipelineLayout(m_device.device(), &layoutInfo, nullptr, &pipelineLayout); result != VK_SUCCESS) {
        PHENYL_LOGE(LOGGER, "Failed to create VkPipelineLayout: {}", result);
        return {};
    }
    PHENYL_DASSERT(pipelineLayout);

    return {pipelineLayout, [&] (VkPipelineLayout&& layout) { m_pipelineLayoutQueue.queueDestruction(layout, getDestructionFrame()); }};
}

VulkanResource<VkPipeline> VulkanResources::makePipeline (const VkGraphicsPipelineCreateInfo& createInfo) {
    VkPipeline pipeline;
    if (auto result = vkCreateGraphicsPipelines(m_device.device(), VK_NULL_HANDLE, 1, &createInfo, nullptr, &pipeline); result != VK_SUCCESS) {
        PHENYL_LOGE(LOGGER, "Failed to create VkPipeline: {}", result);
        return {};
    }
    PHENYL_DASSERT(pipeline);

    return {pipeline, [&] (VkPipeline&& x) { m_pipelineQueue.queueDestruction(x, getDestructionFrame()); }};
}
...
VulkanResource<VkCommandPool> VulkanResources::makeCommandPool (VkCommandPoolCreateFlags usage) {
    ...
    return pool ? VulkanResource<VkCommandPool>{pool, [&] (VkCommandPool&& cmdPool) { m_commandPoolQueue.queueDestruction(cmdPool, getDestructionFrame()); }} : VulkanResource<VkCommandPool>{};
}
...
VulkanResource<VulkanCommandBufferInfo> VulkanResources::makeCommandBuffer (VkCommandPool pool, VkCommandBufferLevel level) {
    ...
    return {VulkanCommandBufferInfo{pool, buffer}, [&] (auto&& info) { m_commandBufferQueue.queueDestruction(info, getDestructionFrame()); } };
}
...
VulkanResource<VkSemaphore> VulkanResources::makeSemaphore () {
    ...
    return {semaphore, [&] (VkSemaphore&& sem) { m_semaphoreQueue.queueDestruction(sem, getDestructionFrame()); }};
}
...
VulkanResource<VkFence> VulkanResources::makeFence (const VkFenceCreateInfo& createInfo) {
    ...
    return {fence, [&] (VkFence&& f) { m_fenceQueue.queueDestruction(f, getDestructionFrame()); }};
}
