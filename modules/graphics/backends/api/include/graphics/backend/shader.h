...
public:
    ...
    [[nodiscard]] virtual std::optional<unsigned int> getSamplerLocation (
        const std::string& sampler) const noexcept = 0;

    virtual std::optional<std::size_t> getUniformOffset (const std::string& uniformBlock,
        const std::string& uniform) const noexcept = 0;
    virtual std::optional<std::size_t> getUniformBlockSize (const std::string& uniformBlock) const noexcept = 0;
};

class Shader : public core::Asset<Shader> {
public:
    Shader () = default;

    explicit Shader (std::unique_ptr<IShader> shader) :
        m_shader{std::move(shader)},
        m_hash{this->m_shader ? this->m_shader->hash() : 0} {}
    ...
    std::optional<std::size_t> uniformOffset (const std::string& uniformBlock,
