...
GlArrayTexture::GlArrayTexture (const graphics::TextureProperties& properties, std::uint32_t texWidth,
    std::uint32_t texHeight) :
    m_sampler{GL_TEXTURE_2D_ARRAY, properties},
    m_width{texWidth},
    m_height{texHeight},
    m_size{0},
    m_capacity{STARTING_DEPTH} {
    // texSampler.bind();
    ...
    //
    // glTexImage3D(texSampler.type(), 0, texSampler.format(), static_cast<GLint>(texWidth),
    // static_cast<GLint>(texHeight), static_cast<GLint>(depthCapacity), 0, texSampler.format(),
...
void GlArrayTexture::reserve (std::uint32_t capacity) {
    ...
    //
    // glTexImage3D(newSampler.type(), 0, newSampler.format(), static_cast<GLint>(width()),
    // static_cast<GLint>(height()), static_cast<GLint>(capacity), 0, newSampler.format(),
    //              newSampler.formatType(), nullptr);
    newSampler.createEmpty3D(width(), height(), capacity);

    glCopyImageSubData(m_sampler.id(), newSampler.type(), 0, 0, 0, 0, newSampler.id(), newSampler.type(), 0, 0, 0, 0,
        static_cast<GLint>(width()), static_cast<GLint>(height()), static_cast<GLint>(m_capacity));
...
void GlArrayTexture::upload (std::uint32_t index, const graphics::Image& image) {
    ...
    PHENYL_ASSERT_MSG(image.width() == width() && image.height() == height(),
        "Invalid image dimensions: array is {}x{} but image is {}x{}", width(), height(), image.width(),
        image.height());

    // glTextureSubImage3D(texSampler.id(), 0, 0, 0, static_cast<GLint>(index),
    // static_cast<GLint>(width()), static_cast<GLint>(height()), 1,
