...
LayoutWidget::LayoutWidget (Axis axis, LayoutArrangement arrangement, LayoutAlignment alignment, const Modifier& modifier) : Widget{modifier}, m_axis{axis}, m_arrangement{arrangement}, m_alignment{alignment} {}
...
Widget* LayoutWidget::insert (std::size_t pos, std::unique_ptr<Widget> child) {
    ...
    m_childOff.emplace(m_childOff.begin() + pos, std::numeric_limits<float>::signaling_NaN(), std::numeric_limits<float>::signaling_NaN());
    ptr->setParent(this);
    return ptr;
}
...
void LayoutWidget::measure (const WidgetConstraints& constraints) {
    ...
    for (auto& i : m_children) {
        ...
        switch (m_alignment) {
            ...
            case LayoutAlignment::END:
                m_childOff.emplace_back(currOff + secondAxis * (remainingSecond + i->modifier().padding) + i->modifier().offset);
                break;
            case LayoutAlignment::CENTER:
                m_childOff.emplace_back(currOff + secondAxis * (remainingSecond / 2 + i->modifier().padding) + i->modifier().offset);
                break;
        }
    ...
    if (m_axis == Axis::UP || m_axis == Axis::LEFT) {
        ...
        for (std::size_t i = 0; i < m_children.size(); i++) {
            m_childOff[i] = secondAxis * glm::dot(m_childOff[i], secondAxis) + mainAxis * (totalLength - glm::dot(m_childOff[i] + m_children[i]->dimensions(), mainAxis));
        }
    }
