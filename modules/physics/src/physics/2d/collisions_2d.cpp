...
static inline Manifold2D buildManifoldInternal (const Face2D& refFace, const Face2D& incFace, glm::vec2 normal,
    float depth) {
    auto dv = incFace.vertices[1] - incFace.vertices[0];
    auto inc1 =
        glm::clamp(lineIntersection(incFace.vertices[0], dv, refFace.vertices[0], refFace.normal), 0.0f, 1.0f) * dv +
        incFace.vertices[0];
    auto inc2 =
        glm::clamp(lineIntersection(incFace.vertices[0], dv, refFace.vertices[1], refFace.normal), 0.0f, 1.0f) * dv +
        incFace.vertices[0];

    // At least one point should not be clipped
    PHENYL_DASSERT(glm::dot(inc1 - refFace.vertices[0], refFace.normal) <= 0 ||
        glm::dot(inc2 - refFace.vertices[0], refFace.normal) <= 0);
...
Manifold2D phenyl::physics::buildManifold (const physics::Face2D& face1, const physics::Face2D& face2, glm::vec2 normal,
    float depth) {
...
Constraint2D Manifold2D::buildConstraint (Collider2D* obj1, Collider2D* obj2, float deltaTime) const {
    ...
    auto elasticityTerm = glm::dot(normal,
        obj1->m_momentum * obj1->m_invMass + r1 * obj1->m_angularMomentum * obj1->m_invInertiaMoment -
            obj2->m_momentum * obj2->m_invMass - r2 * obj2->m_angularMomentum * obj2->m_invInertiaMoment);
...
Constraint2D Constraint2D::ContactConstraint (Collider2D* obj1, Collider2D* obj2, glm::vec2 contactPoint,
    glm::vec2 normal, float bias) {
...
bool Constraint2D::solve () {
    float lambda = -(glm::dot(jVelObj1, obj1->getCurrVelocity()) + glm::dot(jVelObj2, obj2->getCurrVelocity()) +
                       jWObj1 * obj1->getCurrAngularVelocity() + jWObj2 * obj2->getCurrAngularVelocity() + bias) *
        invJacobMass;
