...
bool EpsilonEqual (glm::vec3 a, glm::vec3 b, float epsilon) {
    return glm::epsilonEqual(a[0], b[0], epsilon) && glm::epsilonEqual(a[1], b[1], epsilon) &&
        glm::epsilonEqual(a[2], b[2], epsilon);
}
...
Quaternion::Quaternion (glm::mat3 mat) {
    ...
    // PHENYL_DASSERT_MSG((mat * glm::transpose(mat)) == glm::identity<glm::mat3>(), "Cannot
    // convert non-orthonormal matrix {} to quaternion!", mat);
    PHENYL_DASSERT_MSG(EpsilonEqual((mat * glm::transpose(mat)), glm::identity<glm::mat3>(), 0.00001f),
        "Cannot convert non-orthonormal matrix {} to quaternion!", mat);
    PHENYL_DASSERT_MSG(glm::epsilonEqual(glm::determinant(mat), 1.0f, 0.00001f),
        "Cannot convert non-special orthonormal matrix {} to quaternion (determinant: {})!", mat,
        glm::determinant(mat));
...
Quaternion Quaternion::LookAtOld (glm::vec3 forward, glm::vec3 upAxis) {
    ...
    PHENYL_DASSERT_MSG((glm::abs(glm::dot(glm::normalize(forward), upAxis)) != 1.0f),
        "Up axis cannot be parallel to forward axis");
    ...
    if (trace > 0.0f) {
        ...
        return Quaternion{0.25f / s,
          glm::vec3{rotatedUp.z - forward.y, forward.x - sideAxis.z, sideAxis.y - rotatedUp.x} * s};
    } else if (sideAxis.x > rotatedUp.y && sideAxis.x > forward.z) {
        auto s = 2.0f * glm::sqrt(1.0f + sideAxis.x - rotatedUp.y - forward.z);
        return Quaternion{(rotatedUp.z - forward.y) / s, 0.25f * s, (rotatedUp.x + sideAxis.y) / s,
          (forward.x + sideAxis.z) / s};
    } else if (rotatedUp.y > forward.z) {
        auto s = 2.0f * glm::sqrt(1.0f + rotatedUp.y - sideAxis.x - forward.z);
        return Quaternion{(forward.x - sideAxis.z) / s, (rotatedUp.x + sideAxis.y) / s, 0.25f * s,
          (forward.y + rotatedUp.z) / s};
    } else {
        auto s = 2.0f * glm::sqrt(1.0f + forward.z - sideAxis.x - rotatedUp.y);
        return Quaternion{(sideAxis.y - rotatedUp.x) / s, (forward.x + sideAxis.z) / s, (forward.y + rotatedUp.z) / s,
          0.25f * s};
    }
}
