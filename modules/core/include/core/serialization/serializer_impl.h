...
template<SerializableType T>
ISerializable<std::unordered_map<std::string, T>>& phenyl_GetSerializable (
    SerializableMarker<std::unordered_map<std::string, T>>) {
...
public:
    ...
    void deserializeArray (Vec& obj, IArrayDeserializer& deserializer) override {
        ...
        if (sizeOpt && *sizeOpt != N) {
            throw phenyl::DeserializeException(
                std::format("Invalid length array for {}: expected {}, got {}", name(), N, *sizeOpt));
        }
...
public:
    ...
    void deserializeArray (Mat& obj, IArrayDeserializer& deserializer) override {
        ...
        if (sizeOpt && *sizeOpt != H) {
            throw phenyl::DeserializeException(
                std::format("Invalid length array for {}: expected {}, got {}", name(), H, *sizeOpt));
        }
...
ISerializable<glm::mat<H, W, float>>& phenyl_GetSerializable (SerializableMarker<glm::mat<H, W, float>>) {
    static MatSerializable<float, H, W> serializable{
      H == W ? std::format("glm::mat{}", H) : std::format("glm::mat{}x{}", H, W)};
    return serializable;
}
...
public:
    MethodSerializable (std::string_view name, F1&& getter, F2&& setter) :
        m_name{name},
...
template<typename T, SerializableType B>
    ...
    /*void deserializeObject (T& obj, IObjectDeserializer& deserializer) override {
        ...
        while (deserializer.hasNext()) {
            ...
            if (it != members.end()) {
                ...
                PHENYL_LOGE(SERIALIZER_LOGGER, "Ignoring member \"{}\" in class {}", key,
    name()); deserializer.ignoreNextValue();
            }
        }

        if (deserializedMembers != members.size()) {
            throw DeserializeException(std::format("Failed to deserialize all members of class
    {}", className));
        }
    }*/
        ...
        for (auto& member : m_members) {
            if (!member->deserialize(deserializer, &obj)) {
                throw DeserializeException(
                    std::format("Failed to deserialize member \"{}\" of class {}", member->getKey(), name()));
            }
        }
    }
...
#define PHENYL_SERIALIZABLE_FUNCTION(T, inlineSpecifier, ...)                                               \
    inlineSpecifier ::phenyl::core::ISerializable<T>& phenyl_GetSerializable(                               \
        ::phenyl::core::detail::SerializableMarker<T>) {                                                    \
        using Type = T;                                                                                     \
...
#define PHENYL_SERIALIZABLE_MEMBER_NAMED(member, memberName)                                                           \
    std::make_unique<                                                                                                  \
        ::phenyl::core::detail::MemberSerializable<Type, std::remove_cvref_t<decltype(std::declval<Type>().member)>>>( \
        memberName, &Type::member)
#define PHENYL_SERIALIZABLE_MEMBER(member) PHENYL_SERIALIZABLE_MEMBER_NAMED(member, #member)

#define PHENYL_SERIALIZABLE_METHOD(memberName, getter, setter)                                                       \
    std::make_unique<::phenyl::core::detail::MethodSerializable<Type,                                                \
        std::remove_cvref_t<std::result_of_t<decltype(getter)(const Type*)>>, std::remove_cvref_t<decltype(getter)>, \
        std::remove_cvref_t<decltype(setter)>>>(memberName, getter, setter)

#define PHENYL_SERIALIZABLE_INHERITS_NAMED(Base, baseName) \
    std::make_unique<::phenyl::core::detail::InheritsSerializable<Type, Base>>(baseName)
