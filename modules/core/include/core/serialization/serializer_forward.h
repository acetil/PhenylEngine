...
namespace detail {
    ...
    ISerializable<T>& GetSerializable () {
        using ::phenyl::core::detail::phenyl_GetSerializable;
        return phenyl_GetSerializable(SerializableMarker<T>{});
    }
} // namespace detail

template <typename T> concept SerializableType = requires {
    { detail::GetSerializable<T>() } -> std::same_as<ISerializable<T>&>;
};
} // namespace phenyl::core

#define PHENYL_DECLARE_SERIALIZABLE(T) \
    ::phenyl::core::ISerializable<T>& phenyl_GetSerializable(::phenyl::core::detail::SerializableMarker<T>);
#define PHENYL_SERIALIZABLE_INTRUSIVE(T) \
    friend ::phenyl::core::ISerializable<T>& phenyl_GetSerializable(::phenyl::core::detail::SerializableMarker<T>);
