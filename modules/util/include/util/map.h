...
namespace detail {
    ...
    public:
        ...
        BasicMapIterator (detail::MapPair<K, V>* _data, std::size_t _maxSize, std::size_t _pos = 0) :
            data{_data},
            maxSize{_maxSize},
            pos{_pos} {
    ...
    public:
        ...
        BasicMapKVIterator (detail::MapPair<K, V>* _data, std::size_t _maxSize, std::size_t _pos = 0) :
            data{_data},
            pos{_pos},
            maxSize{_maxSize} {
            while (pos < maxSize && !data[pos].exists()) {
...
protected:
    ...
    void resizeContainer () {
        std::unique_ptr<std::pair<char, K>[]> newExists(
            std::unique_ptr<std::pair<char, K>[]>(new std::pair<char, K>[maxSize * 2]));
...
public:
    ...
    Map2 (const Map2<K, V, hashFunc>& other) :
...
public:
    ...
    // Iterator
    MapIterator (const MapIterator<K, V, Wrapper>& it) :
        exists(it.exists),
        data(it.data),
        maxSize(it.maxSize),
        index(it.index) {}
    ...
    friend bool operator== (const iterator& it1, const iterator& it2) {
        return (it1.index >= it1.maxSize && it2.index >= it2.maxSize) ||
            (it1.data == it2.data && it1.index == it2.index);
    }
    ...
    MapIterator(const Map2<K, V, HashFunc>& map, std::size_t startIndex) :
        maxSize{map.maxSize},
        index{startIndex},
        exists(map.exists.get()),
        data{(V*) map.data.get()} {
        nextIndex();
    }
};
