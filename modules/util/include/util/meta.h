...
constexpr bool is_callable_list (F&&) {
    return type_list_unroll<can_call, list_add_front<F, L>>{};
}
...
struct is_in_impl {
    ...
    using val =
        typename std::disjunction<std::is_same<get_nth_typelist<N - 1, L>, T>, typename is_in_impl<N - 1, T, L>::val>;
};
...
private:
    ...
    struct inner {
        ...
        static constexpr char SFINAE (Template<Args...>* t) {
            return 'a';
        }

        template<template<typename...> typename Template>
        static constexpr short SFINAE (...) {
            return 0;
        }
    };
...
private:
    template<typename F, typename... Args>
    static constexpr auto SFINAE (char* t)
        -> std::enable_if_t<std::is_same_v<decltype(std::declval<F>()(std::declval<Args>()...)),
...
template<typename T, typename... Args> concept IsInPack = is_in_pack<T, Args...>;
...
template<typename F, typename R, typename... Args> concept callable = requires(F fn, Args... args) {
