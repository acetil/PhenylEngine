...
static_assert(PHENYL_MIN_LOG_LEVEL <= LEVEL_FATAL);
    ...
    #define PHENYL_LOGD_IF(cond, logger, fmt, ...) \
        if (cond)                                  \
        PHENYL_LOGD(logger, fmt __VA_OPT__(, ) __VA_ARGS__)
#else
    ...
    #define PHENYL_LOGI_IF(cond, logger, fmt, ...) \
        if (cond)                                  \
        PHENYL_LOGI(logger, fmt __VA_OPT__(, ) __VA_ARGS__)
#else
    ...
    #define PHENYL_LOGW_IF(cond, logger, fmt, ...) \
        if (cond)                                  \
        PHENYL_LOGW(logger, fmt __VA_OPT__(, ) __VA_ARGS__)
#else
    ...
    #define PHENYL_LOGE_IF(cond, logger, fmt, ...) \
        if (cond)                                  \
        PHENYL_LOGE(logger, fmt __VA_OPT__(, ) __VA_ARGS__)
#else
    #define PHENYL_LOGE(logger, fmt, ...)
    #define PHENYL_LOGE_IF(cond, logger, fmt, ...)
#endif

#define PHENYL_LOGF(logger, fmt, ...)                                                  \
    do {                                                                               \
        logger.fatal(std::source_location::current(), fmt __VA_OPT__(, ) __VA_ARGS__); \
        ::phenyl::PrintStackTrace();                                                   \
    } while (0)
#define PHENYL_LOGF_IF(cond, logger, fmt, ...) \
    if (cond)                                  \
    PHENYL_LOGF(logger, fmt __VA_OPT__(, ) __VA_ARGS__)

#define PHENYL_ASSERT(cond)                                                                                    \
    do {                                                                                                       \
        if (!(cond)) {                                                                                         \
            auto sourceLoc = std::source_location::current();                                                  \
            ::phenyl::PHENYL_LOGGER.fatal(sourceLoc, "{}({}:{}): assertion failed: {}", sourceLoc.file_name(), \
                sourceLoc.line(), sourceLoc.column(), #cond);                                                  \
            ::phenyl::PrintStackTrace();                                                                       \
            std::terminate();                                                                                  \
        }                                                                                                      \
    } while (0)

#define PHENYL_ASSERT_MSG(cond, fmt, ...)                                                                          \
    do {                                                                                                           \
        if (!(cond)) {                                                                                             \
            auto sourceLoc = std::source_location::current();                                                      \
            ::phenyl::PHENYL_LOGGER.fatal(sourceLoc, "{}({}:{}): assertion failed: {}: {}", sourceLoc.file_name(), \
                sourceLoc.line(), sourceLoc.column(), #cond, std::format(fmt __VA_OPT__(, ) __VA_ARGS__));         \
            ::phenyl::PrintStackTrace();                                                                           \
            std::terminate();                                                                                      \
        }                                                                                                          \
    } while (0)

#define PHENYL_ABORT(fmt, ...)                                                                          \
    do {                                                                                                \
        auto sourceLoc = std::source_location::current();                                               \
        ::phenyl::PHENYL_LOGGER.fatal(sourceLoc, "{}({}:{}): runtime abort: {}", sourceLoc.file_name(), \
            sourceLoc.line(), sourceLoc.column(), std::format(fmt __VA_OPT__(, ) __VA_ARGS__));         \
        ::phenyl::PrintStackTrace();                                                                    \
        std::terminate();                                                                               \
    } while (0)
        ...
        #define PHENYL_TRACE(logger, fmt, ...) \
            logger.trace(std::source_location::current(), fmt __VA_OPT__(, ) __VA_ARGS__)
        #define PHENYL_TRACE_IF(cond, logger, fmt, ...) \
            if (cond)                                   \
            PHENYL_TRACE(logger, fmt __VA_OPT__(, ) __VA_ARGS__)
    #else
        #define PHENYL_TRACE(logger, fmt, ...)
        #define PHENYL_TRACE_IF(cond, logger, fmt, ...)
    #endif
