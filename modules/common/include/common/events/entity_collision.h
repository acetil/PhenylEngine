...
namespace event {
    ...
    public:
        ...
        EntityCollisionEvent (component::Entity firstEntity, component::Entity secondEntity, unsigned int _collisionLayers,
                              component::EntityComponentManager& compManager, EventBus::SharedPtr bus) : firstEntity(std::move(firstEntity)),
                                    secondEntity(std::move(secondEntity)), collisionLayers(_collisionLayers), componentManager(compManager), eventBus(std::move(bus)) {};
    };
}
